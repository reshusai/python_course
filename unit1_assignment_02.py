...
def get_lower_to_upper_dict():
    ...
    """
    input1 = list(map(chr,range(97,123)))
    input2 = list(map(chr,range(65,91)))
...
def get_encoding_dict():
    ...
    input1 = list(map(chr,range(97,123)))
    input2 = list(map(chr,range(65,91)))
    input1 = list(chain(input1,input2))
    input2=list(range(1,27))
    input2=list(chain(input2,input2))
    return dict(zip(input1,input2))
...
def get_consonents(input):
    ...
    No control flow plz.
    """
    return set(filter(lambda x:x not in VOWEL_SET, (input.lower())))
...
def capitalize_names(input):
    ...
    """
    return  list(map(lambda x:(x.lower()).capitalize(),input))
