...
def test_int_iterable():
    try:
        iter(10)
    except Exception:  # replace by appropriate except so this test passes
        pass
...
def test_find_builtins_that_work_on_iterables():
    funcs = ["iter","max","map","all","any","dict","enumerate","filter","frozenset","list","min","next","set","sorted","sum","tuple","zip"]
    assert 17 == len(funcs)
