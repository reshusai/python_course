...
def test_raw_strings_are_strings():
    ...
    assert 2 == len(r"\n")    #printf \n it is 2 char so len is 2.


    # write a non-raw string in the ___ to make both strings equal.
    assert 'Hello\\n' == r"Hello\n"   #we can use both r or the \\ for the same result. \\ is non-raw string
...
def rotate_right(input, count):
    ...
    """
    return input[-count:] + input[0:-count]  # replace __ with the code
...
- raw string 'r' is used to printf \ characters as it is. And 'r' is equal to '\\'
- slicing helps to get the substring and negative indication in slicing gives the total length - number
- using f implicilty gives u the best runtime result with the variables 
"""
