...
def test_numbers_arithmetic_operations():
    ...
    assert -10 == 10 - 20
    assert 2.0 == 6/3        #returns float value

    # observe the difference between / and //
    assert 2 == 7 // 3
    assert -3 == -7 // 3      #returns int value
    assert -3 == 7 // -3
    ...
    #>>> import math
    #>>> help(math.isclose)
    assert math.isclose(2.33, 7/3, abs_tol=0.01)
    """fix abs_tol so that this assert passes"""
...
- ** used as the power symbol
- / will return float value and // returns int value
- abs_tol is used to fix the precision.
"""
