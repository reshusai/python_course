...
def test_function_call_with_keyword_arguments():
    ...
    assert [90,20,30,4] == demo(90,second=20, third=30)
    assert [10,60,30,4] == demo(first=10,second=60, third=30)
    assert [10,50,30,4] == demo(10,50, third=30)
    assert [100,20,3,4] == demo(100,second=20)  # is this allowed? correct and uncomment
...
def simple_format(format, *args):
    ...
    """
    return ' '.join([args[int(x[1:])] if (x[0] == "%" and int(x[1:])<len(args)) else x for x in format.split()])
